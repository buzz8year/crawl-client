<?php

namespace backend\models\parser;

/**
 * ParserEstablishingInterface defines mixed functionality setting (establishing) the action stage for the master plan.
 */
interface ParserInterface
{

    // public function createModel(int $sourceId);
    // public function getClass(int $sourceId);

    public function curlSession(string $url);
    public function parse(string $type, string $url, string $page);
    public function getNodes(string $response, string $xpath);

}
