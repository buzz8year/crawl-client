...
class KeyParser extends Parser implements ParserSourceInterface
{
    ...
    public function getImageData($object)
    {
        ...
        foreach ($object as $node) {
            $data[] = [
                'fullsize' => $this->handleUrl($node->getAttribute('data-bigphoto')),
                'thumb'    => $this->handleUrl($node->getAttribute('data-preview'))//div[@id=\'description\'],
            ];
        }
        return $data;
    }
    ...
    public function buildUrl(string $regionSourceId = '', string $categorySourceId = '', string $keyword = '')
    {
        ...
        if ($categorySourceId && $keyword) {
            $url = $this->handleUrl($category->source_url) . '?' . self::QUERY_KEYWORD . $keyword;
        }
