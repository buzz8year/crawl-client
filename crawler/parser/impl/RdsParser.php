...
use crawler\parser\interface\ParserSourceInterface;
...
class RdsParser extends Parser implements ParserSourceInterface
{
     ...
     * @return object
     */
    public function getSuperData($nodes): array {
        return [];
    }
    ...
    public function buildUrl(string $regionSourceId = '', string $categorySourceId = '', string $keyword = ''): string
    {
    ...
    public function parseCategories(): array
    {
        ...
        if ($response) 
        {
            $nodes = $this->dom->getNodes($response, self::CATEGORY_WRAP_NODE);
