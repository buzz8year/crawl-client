...
class ParserProvisioner implements ParserProvisioningInterface
{
    ...
    public static function activeCategories(int $sourceId)
    {
        ...
        $maxNest = \crawler\models\category\CategorySource::find()
            ->where('source_id = ' . $sourceId)
            ->max('nest_level');

        $categories = \crawler\models\category\Category::find()
        ...
        foreach ($categories as $category) 
        {
            if ($category['source_url']) 
            {
                ...
                $children = \crawler\models\category\Category::find()
                    ->select(['category.id as id', 'cs.id as csid', 'category.title', 'cs.source_url'])
                    ->join('join', 'category_source cs', 'cs.category_id = category.id')
    ...
    public function listSourceKeywords(int $sourceId)
    {
        ...
        $keywords = \crawler\models\keyword\Keyword::find()
    ...
    public static function buildTree(array $categories, int $parentId = null)
    {
        ...
        foreach ($categories as $category) 
        {
            ...
            {
                $tree[$category['id']] = \crawler\models\category\Category::find()
                    ->select('*')
                    ->join('join', 'category_source cs', 'cs.category_id = category.id')
     ...
     * @inheritdoc
     */
    public static function displayNestedSelect(array $categories, int $sourceId, string $regionId = '', int $currentCategoryId = 0)
    {
        // $html = '';
