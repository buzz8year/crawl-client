...
namespace crawler\models\morph;
...
Class Morph 
{
    ...
    public function lemmatizeOne(string $word) 
    {
        try {
            ...
            if ($lemmas && !in_array($speech[0], ['СОЮЗ','ПРЕДЛ'])) 
            { // SPEECH: сущ., прил., гл. 
                if (count($lemmas) > 1) 
                {
                ...
                else return $lemmas[0]; // TODO: It should broken after first match down the getBaseForm() method and not here, if performance does count
            }
